            ...
            while not self.queue_cmd.empty():
                def process_cmd(msg):
                  ...
                  if self.cmd_parser.commandString == self.command.CMD_LED:
                      ...
                      elif self.cmd_parser.commandString == self.command.CMD_PAUSE:
                          time.sleep(self.cmd_parser.intParameter[0]/1000)
                ...
                if msg.startswith(self.command.CMD_MULTI):
                  ...
                  self.cmd_parser.parser(submsgs[0])
                  if self.cmd_parser.commandString == self.command.CMD_START_REPEATS:
                    repeats = int(self.cmd_parser.intParameter[0])
                    print("REPEATING: {}".format(repeats))
                    submsgs = submsgs[1:]

                  # for submsg in msg[len(self.command.CMD_MULTI):].split('$')[1:]
                  while repeats:
                    repeats -= 1
                    print("START REPEAT")
                  ...
                  if BLOCKING:
                    ...
                    self.tcp_server.sendDataToCmdClinet("DONE")
